...
get_population <- function(region) {
  ...
  v %>% 
    ...
    mutate(
      ...
      hispanic = 
        population_pct(group = B03001_003E, total = B03001_001E),
      asian_nonhispanic = 
        population_pct(group = B03002_006E, total = B03001_001E),
      black_nonhispanic = 
        population_pct(group = B03002_004E, total = B03001_001E),
      other =
      ...
      population_most   =
        ...
        pmap_chr(population_most) %>% 
        if_else(population == 0L, "uninhabited", .)
    ) %>% 
    select(geoid = GEOID, name = NAME, population:population_most) %>% 
    arrange(geoid)
}
...
for (region in names(files_out)) {
  get_boundaries(region) %>% 
    ...
    arrange(geoid) %>% 
    select(name, population, density, white_nonhispanic:population_most) %>% 
